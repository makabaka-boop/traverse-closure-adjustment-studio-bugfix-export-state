...
- **共享 / 下载画布**：支持 Web Share（含文件）时调系统分享面板，否则自动降级下载 PNG；
  两条出口均纯前端、离线可用。分享被用户取消时如实提示“已取消，未产生任何文件”，
  不会谎报已下载；画布编码为空（零尺寸/空白）时如实提示失败且不产生下载。
  图形区域隐藏或尺寸为零时画布不会落笔，共享与 PNG 入口同步不可用；
  成功提示均标注对应结果（边数与两轴闭合差），可与下载 JSON 逐位核对，
  保证交付的 PNG 一定来自当前平差结果。
...
- 导出可观察状态：可控分享拒绝（取消不谎报下载）、空画布编码（如实失败）、
  零尺寸到恢复绘制的顺序（恢复前导出入口关闭、恢复后 PNG 对应当前结果），
  并核对提示文案与实际下载次数。
